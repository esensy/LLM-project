...
def merge_for_mup(mup_path, output_file="merged_mup_data.json"):
...
def merge_for_plumber(plumber_path, output_file="merged_plumber_data.json"):
...
def run_preprocessing(pymupdf_path, pdfplumber_path):
    """
    Run the complete preprocessing pipeline for both PyMuPDF and PDFPlumber data.
    
    Args:
        pymupdf_path (str): Path to PyMuPDF JSON files
        pdfplumber_path (str): Path to PDFPlumber JSON files
    """
    # 로깅 설정
